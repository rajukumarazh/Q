...
var cors = require('cors');
app.use(cors());
const attempted = require('./mongo/attemped_data');
...
app.post('/addongoig_course', (req, res) => {
	let { course_id, user_id } = req.body;
	const data = new attempted({
		user_id: user_id,
		course_id: course_id,
	});
	const dataToSave = data.save();
...
app.post('/getongoing', async (req, res) => {
	console.log('req,', req.body);
	// let dt = await attempted.findOne({ user_id: req.body.user_id });
	attempted.findOne({ user_id: req.body.user_id }, function (err, docs) {
		if (err) {
			console.log(err);
		} else {
			res.send({ data: docs });
